    ...
    static convertNode(nodeData, data) {
        ...
        if (nodeData.repeat && data[nodeData.repeat]) {
            ...
            repeatedData.forEach(dataItem => {
                html += TemplateEngine.convertNode(nodeData, dataItem);  // Pass the specific item data here
            });
        } else {
            html = `<${nodeData.tag}`;
            ...
            if (nodeData.attributes) {
                for (const attrName in nodeData.attributes) {
                    const attrValue = TemplateEngine.interpolate(nodeData.attributes[attrName], data);
            ...
            if (nodeData.content) {
                html += TemplateEngine.interpolate(nodeData.content, data);
            }
            ...
            if (nodeData.children && nodeData.children.length > 0) {
                ...
                nodeData.children.forEach((childNode) => {
                    ...
                    html += TemplateEngine.convertNode(childNode, data);  // Pass the original data here
                });
            }
